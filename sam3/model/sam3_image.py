...
class Sam3Image(torch.nn.Module):
    ...
    def _encode_prompt(
        ...
        txt_ids = find_input.text_ids  # 객체에서 현재 배치에 필요한 텍스트 프롬프트의 ID를 가져옵니다.
        txt_feats = backbone_out["language_features"][:, txt_ids] # 텍스트 임베딩 추출 (텍스트 인코더를 통해 얻어진 특징 맵에서 해당 ID에 해당하는 부분만 선택)
        txt_masks = backbone_out["language_mask"][txt_ids] # 텍스트의 실제 길이(패딩이 아닌 부분)를 나타내는 마스크

        feat_tuple = self._get_img_feats(backbone_out, find_input.img_ids)  # backbone_out에서 이미지 특징, 위치 임베딩, 특징 맵의 크기를 추출
        backbone_out, img_feats, img_pos_embeds, vis_feat_sizes = feat_tuple

        if prev_mask_pred is not None: # 만약 이전 단계에서 예측된 마스크가 있다면 이 마스크를 최종 이미지 특징맵에 더하여 이미지 특징을 업데이트
        ...
        )  # 사용자가 제공한 기하학적 프롬프트(예: 바운딩 박스)를 인코딩하여 기하학적 특징과 마스크를 생성, 이 기하학적 프롬프트를 벡터 특징과 마스크로 변환
        ...
        # 텍트스 특징, 기하학적 특징, 시각적 프롬프트를 결합하여 최종 프롬프트와 마스크 생성
        if encode_text:
    ...
    def forward_grounding(
        ...
        out = {
            "encoder_hidden_states": encoder_out["encoder_hidden_states"],  # 트랜스포머 인코더를 통과하여 프롬프트(텍스트, 기하학정 정보)의 영향을 받아 개선되고 컨텍스트화된 최종 이미지 특징 맵
            "prev_encoder_out": {  # 이전 인코더 출력 (디버깅 또는 분석용, 단일 이미지 추론에서는 필요 없음)
